...
public class ExportedDetailedInvoiceDAO implements DAO<DetailedInvoice> {

    public static ExportedDetailedInvoiceDAO getInstance() {
        return new ExportedDetailedInvoiceDAO();
    }

    @Override
    public int insert(DetailedInvoice t) {
        int result = 0;
        String sql = "INSERT INTO ExportedDetailedInvoice (invoiceId, productId, quantity, price) VALUES(?,?,?,?)";
        try {
    ...
    public int update(DetailedInvoice t) {
        int result = 0;
        String sql = "UPDATE ExportedDetailedInvoice SET invoiceId=?, productId=?, quantity=?, price=? WHERE invoiceId=? AND productId=?";
        try {
            Connection con = JDBCUtil.getConnection();
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getInvoiceId());
    ...
    public int delete(DetailedInvoice t) {
        int result = 0;
        String sql = "DELETE FROM ExportedDetailedInvoice WHERE invoiceId=? ";
        try {
            Connection con = JDBCUtil.getConnection();
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, t.getInvoiceId());
    ...
    public ArrayList<DetailedInvoice> selectAll() {
        ...
        String sql = "SELECT * FROM ExportedDetailedInvoice";
        try {
            Connection con = JDBCUtil.getConnection();
            PreparedStatement pst = con.prepareStatement(sql);
    ...
    public ArrayList<DetailedInvoice> selectAll(String ID) {
        ArrayList<DetailedInvoice> result = new ArrayList<>();
        String sql = "SELECT * FROM ExportedDetailedInvoice WHERE invoiceId=?";
        try {
            Connection con = JDBCUtil.getConnection();
            PreparedStatement pst = con.prepareStatement(sql);
            pst.setString(1, ID);
